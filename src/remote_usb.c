...
static void process_standard_request(const struct UsbSetupPacket_t* req) {
  ...
  switch (req->bRequest) {
    case GET_STATUS:
      ...
      if (valid_request) {
        clear_in();
        while (!(UEINTX & _BV(RXOUTI))) {} // FIXME blocking code
        clear_out();
      }
      break;
    case SET_ADDRESS:
    ...
    if (req->bRequest == VENDOR_REQUEST_PUSH_FRAME && req->wLength == FRAME_LENGTH) {
      ...
      if (usb_frame) {
        ...
        while (write_ptr != end_ptr) {
          while (!(UEINTX & _BV(RXOUTI))) {} // FIXME blocking code: transactionise
...
/*  if (FLAG_IS_SET(UEINT, 1)) {*/
/*    UENUM = 1;*/
/*    if (FLAG_IS_SET(UEIENX, RXOUTE) && FLAG_IS_SET(UEINTX, RXOUTI)) {*/
/*      // TODO Get data and move it to a framebuffer*/
/*      CLEAR_INTERRUPT(UEINTX, RXOUTI);*/
/*    }*/
/*  }*/
